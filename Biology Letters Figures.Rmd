...
Compiling all the data and code required to produce the figures for manuscript in prep. for Biology Letters.
...
```{r}
png_1 <- readPNG("C:/Users/kjmurphy/OneDrive - University of Tasmania/Thesis Figures/Conceptual Diagram/Fig_1_Panel_A_v1.png")
...
Figure 2: Ecosystem metrics - Biomass, turnover, stability (1/CV), productibity


## Figure XX: Predator-prey size relationships

```{r}
df_combined <- read.csv("df_Figure_1_Trophic_Allometry_Model_Results.csv")
  ...
  df_RTP_plot <- df_combined
  
# df_RTP_plot <- read.csv("RTP_plot_F+G_combined_results.csv")
  
RTP_plot <-
  df_RTP_plot %>%
    mutate(Func_Group = recode(Func_Group, "Low Activity" = "low activity","High Activity" ="high activity",
                              "Zooplankton" = 'zooplankton', "Fish" = "fish")) %>% 
...
PPMR_plot <- df_PPMR_plot %>% 
  ...
  #             linetype = "dotted", size = 0.75) +
    geom_line(size = 1, show.legend = F) +
     labs(col = "functional group")+
    # geom_segment(aes(x = min(Predator_Size), xend = max(Predator_Size), y = (min(Predator_Size)*0.005882353), yend = (max(Predator_Size)*0.005882353)), linetype = "dashed", colour = "black", size=0.5) +
    geom_segment(aes(x = min(Predator_Size), xend = max(Predator_Size), y = (min(Predator_Size)* 0.01010101), yend = (max(Predator_Size)* 0.01010101)), linetype = "dashed", colour = "dark grey", size=0.5) +
    geom_segment(aes(x = min(Predator_Size), xend = max(Predator_Size), y = (min(Predator_Size)*  0.0009775171), yend = (max(Predator_Size)*  0.0009775171)), linetype = "dashed", colour = "black", size=0.5) + ## Barnes dataset realised PPMR value for ectotherm vertebrates
   ...
   scale_fill_manual(values = c("#CCCCCC" ,  "#01cbfeff","#FE5455ff","#666333")) +
    ...
    annotate(angle = 33, "text", label= "PPMR = 99", x = 10^-10.2, y = 10^-11.7, size = 4, colour = "dark gray",fontface = 2) + # If using Barnes realised PPMR for cephalopods of 99
    annotate(angle = 33, "text", label= "PPMR = 1023", x = 10^-7.5, y = 10^-10, size = 4, fontface = 2) + # If using Barnes realised PPMR for cephalopods of 1023 ectotherm vertebrates
...
# ggsave("Figure_XX_Bio_Letters_180_180mm.png",
#        units="mm",
#        dpi = 600,
#        width = 180,
#        height = 180)

# ggsave("Figure_XX_Bio_Letters_300_250mm.png",
#        units="mm",
#        dpi = 600,
#        width = 300,
#        height = 250)
...
ggsave("Figure_XX_Bio_Letters_200_200mm.png",
       units="mm",
       dpi = 600,
       width = 200,
       height = 200)
...
df_Eco_Plot %>%
  ...
  # filter(Metric == "Total Biomass (g m3)" | Metric == "Fish:Community Biomass"| Metric == "Fish P:B"| Metric =="Biomass weighted PPMR"|Metric == "Biomass weighted TTE"|Metric == "Fish Biomass (g m3)") %>%
  filter(Metric == "Mean P:B") %>%
ggplot(aes(x = Model, y=(1/Value), fill = Model), width =2) +
  scale_fill_manual(values = c("grey" , "orange")) +
  scale_x_discrete(labels= Model_Sets) +
  # labs(title = "Dynamic Repro") +
  geom_violin(show.legend = F, lwd=.3) +
  stat_summary(fun.y="median", shape=23, 
                 geom="point", size=2,show.legend = F) +
  labs(tag = "E") +
  ylab("Turnover Time (years)")+
