...
#Extract biomass weighted emergent PPMR, using the biomass and relative trophic position dataframes
ppmr_biomass_weighted_Basic_Model <-function(logbase, df_RTP, df_B){
  ...
  weighted_slope <- (ppmr_fish*df_B_fish$Biomass)+(ppmr_zoo*df_B_zoo$Biomass)
...
ppmr_spp_avg_Simple <-function(logbase, df_RTP){
  ...
  avg_PPMR <- (ppmr_fish + ppmr_zoo)/2
  
  names(avg_PPMR) <- "Species_Avg_PPMR"
  
  return(avg_PPMR)
  
}

ppmr_spp_Simple <-function(logbase, df_RTP){
...
ppmr_spp_avg <-function(logbase, df_RTP){
  ...
  avg_PPMR <- (ppmr_active + ppmr_inactive + ppmr_fish + ppmr_zoo)/4
...
ppmr_spp <-function(logbase, df_RTP){
...
Norm_Biomass_Spectra_Function_BioLetters <- function(projection){
  ...
  df_combined <- left_join(
    ...
    select(df_size_classes, Size, Size_Class, bin_width),
    by="Size_Class") %>%
    # then join df_species to df_combined so that there is a variable with actual species names rather than a numeric variable 1:n (in this case 1:7)
  ...
  df_norm_biomass_tmax <- df_combined %>%
    filter(Time == tmax) %>%
    group_by(Species,Size) %>%
    mutate(Biomass = N*Size)
  
  df_norm_biomass_tmax <- df_norm_biomass_tmax %>%
    filter(N > 0) %>%
    group_by(Species, Size) %>%
    mutate(Norm_biomass =Biomass/bin_width) %>%
    mutate(Log_Nbiomass = log(Norm_biomass))
  
  ## join df_N with df_size_class data in order to create log10 size classes rather then the size     class integer 1:184 that is in df_N
  
  df_norm_biomass_tavg  <- df_combined %>%
    filter(Time >= I(tmax - cutoff_time ), Time <= tmax) %>%
    group_by(Species, Size, bin_width) %>%
    summarise(Mean_N = mean(N)) %>%
    mutate(Biomass = Mean_N*Size) %>%
    mutate(Norm_biomass = Biomass/bin_width) %>%
    mutate(Norm_N = Mean_N/bin_width) %>%
    filter(Norm_biomass > 10^-10)
  
  df_norm_biomass_tavg_All_Spp_Avg  <- df_combined %>%
    filter(Time >= I(tmax - cutoff_time ), Time <= tmax) %>%
    group_by(Size, bin_width) %>%
    summarise(Mean_N = mean(N)) %>%
    mutate(Biomass = Mean_N*Size) %>%
    mutate(Norm_biomass = Biomass/bin_width) %>%
    mutate(Norm_N = Mean_N/bin_width) %>%
  ...
  df_norm_biomass_tavg$Species_Order <- factor(df_norm_biomass_tavg$Species, 
  ...
  df_facet <- df_norm_biomass_tavg
  df_facet <- transform(df_facet, log_Norm_Biomass = log10(Norm_biomass))
  df_facet <- transform(df_facet, log_Size = log10(Size))
  
  # df_norm_biomass_mod_output <- df_facet
  # df_norm_biomass_mod_output <<- df_norm_biomass_mod_output
  
  dat <- df_facet
  group <- df_facet$Species_Order
  yvar <- "log_Norm_Biomass"
  xvar <- "log_Size"
  ...
  # ggplot(df_norm_biomass_tavg, aes_string(x = names(fit$model)[2], y = names(fit$model)[1], colour = as.factor(group))) + 
  #   geom_smooth(size=1, alpha=F, method="loess") +
  #   xlab(expression(Size~(10^X~g))) +
  #   ylab("Normalised Biomass (log10)")+
  ...
  #   geom_segment(aes(x = projection$environ$phyto_min , xend = projection$environ$phyto_max, y = (projection$environ$a) + (projection$environ$b*projection$environ$phyto_min), yend = projection$environ$a + (projection$environ$b*projection$environ$phyto_max)), colour="green",linetype="longdash", size = 1) +
  ...
  df_facet_spp_avg <- df_norm_biomass_tavg_All_Spp_Avg
  df_facet_spp_avg <- transform(df_facet_spp_avg, log_Norm_Biomass = log10(Norm_biomass))
  df_facet_spp_avg <- transform(df_facet_spp_avg, log_Size = log10(Size))
  
  # df_norm_biomass_mod_output <- df_facet
  # df_norm_biomass_mod_output <<- df_norm_biomass_mod_output
  
  dat_v1 <- df_facet_spp_avg
  # group <- df_facet$Species_Order
  yvar <- "log_Norm_Biomass"
  xvar <- "log_Size"
  ...
  fit_v1 <- lm(fml, dat_v1)
  
  ggplot(df_norm_biomass_tavg, aes(x =log10(Size), y = log10(Norm_biomass), colour = as.factor(Species_Order))) + 
    geom_path(size=0.5) +
    xlab(expression(Size~(10^X~g))) +
    ylab("Normalised Biomass (log10)")+
    #geom_point(aes(x=log(df_mode_norm_biomass$size_class), y=log(df_mode_norm_biomass$normalised_biomass), colour = df_mode_norm_biomass$Feeding_mode)) +
...
Norm_Biomass_Spectra_Function_BioLetters_Simple <- function(projection){
  ...
  ggplot(df_norm_biomass_tavg, aes(x =log10(Size), y = log10(Norm_biomass), colour = as.factor(Species_Order))) + 
    ...
    theme_classic() +
    scale_colour_manual(name = 'Functional Group',
                        values =c("#C77CFF", "#7CAE00"), labels = c('Zooplankton','Fish')) +
    theme(plot.title = element_text(size = 12)) +
    theme(
      axis.title = element_text(size=12),
      axis.text.x=element_text(colour="black", size = 10),
...
TL_Allom_Plot_Inde_Scaling<- function(projection){
  ...
  #   ggplot(aes(x=Predator_Size, y= RTP, colour=name)) +
  #   scale_colour_manual(values = c( "#F8766D", "#00BFC4","#C77CFF", "#7CAE00")) +
  #   geom_smooth(method = "loess", size=1.5, alpha = 0, linetype = "dashed") +
  #   #facet_wrap(~Species_Order, scales = "free_x") +
  #   geom_point(aes(x=Predator_Size, y= RTP, colour=name),data = df_emp_active, size = .5) +
...
TL_Allom_Plot <- function(df_combined){
...
TTE_biomass_weighted <-function(logbase, df_RTP, df_B){
  ...
  tte_active <- ppmr_active^(-1.05 + 0.75)
  tte_inactive <- ppmr_inactive^(-1.05 + 0.75)
  tte_fish <- ppmr_fish^(-1.05 + 0.75)
  tte_zoo <- ppmr_zoo^(-1.05 + 0.75)
    
  weighted_spp <- (tte_active*df_B_active$Biomass)+(tte_inactive*df_B_inactive$Biomass)+(tte_fish*df_B_fish$Biomass)+(tte_zoo*df_B_zoo$Biomass)
  
  tte_B_weighted <- weighted_spp/df_B_sum_total$Total_biomass
  
  names(tte_B_weighted) <- "Biomass_weighted_TTE"
  
  return(tte_B_weighted)
  
}

TTE_biomass_weighted_Simple <-function(logbase, df_RTP, df_B){
  ...
  # weighted_spp <- (tte_active*df_B_active$Biomass)+(tte_inactive*df_B_inactive$Biomass)+(tte_fish*df_B_fish$Biomass)+(tte_zoo*df_B_zoo$Biomass)
  weighted_spp <- (tte_fish*df_B_fish$Biomass)+(tte_zoo*df_B_zoo$Biomass)
...
N_to_Biomass <- function(projection){
  ...
  df_biomass_int$Predator <- factor(df_biomass_int$Predator, 
                                             levels = c('Zooplankton','Active Cephs',
                                                        'Inactive Cephs','Fish'),
                                             ordered = TRUE)
  
  p1 <- ggplot(df_biomass_int, aes(x=Time, y= Biomass_sum, colour=Predator)) +
    geom_point() +
    scale_colour_manual(values = c("#7CAE00","#F8766D", "#00BFC4",  "#C77CFF")) +
    xlab("Year") +
    facet_wrap(~Predator) +
  ...
  p2 <-  ggplot(df_biomass_int, aes(x=Time, y= Biomass_sum, colour=Predator)) +
    geom_point() +
    scale_colour_manual(values = c("#7CAE00","#F8766D", "#00BFC4",  "#C77CFF")) +
    xlab("Year") +
...
Compute_CV_Simple <- function(df, cutoff_time){
  
  tmax <- length(df$Annual_Biomass)/2
  ...
  # CV_list <- list(CV_community, CV_Zoo, CV_Fish)
  # 
  # return(CV_list)
